...
CREATE UNIQUE INDEX IF NOT EXISTS idx_profiles_created_at ON profiles.profiles (created_at );
CREATE UNIQUE INDEX IF NOT EXISTS idx_profiles_user_id ON profiles.profiles(user_id);
create index idx_profiles_geo_city_id on profiles.profiles (city_id);
...
(
    ...
    guid_id          UUID   NOT NULL,
    profile_id          bigint
        constraint fk_ofices_profile_id
        references profiles.profiles
        on delete restrict,
...
CREATE INDEX IF NOT EXISTS idx_locations_profile_id ON profiles.locations (created_at);
...
create index idx_locations_geo_city_id on profiles.locations (city_id);
CREATE INDEX idx_locations_location ON profiles.locations USING GIST (location);
...
CREATE INDEX IF NOT EXISTS idx_portfolio_profile_id ON profiles.portfolio (created_at);
...
(
    ...
    brand_id bigint not null
        ...
        references products.brands
        on delete restrict,
    constraint pk_portfolio_brands primary key (profile_id, brand_id)
    );
